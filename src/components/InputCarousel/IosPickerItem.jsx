...
export const getSlidesStyles = (emblaApi, loop, slideCount, totalRadius) => {
  ...
  for (let index = 0; index < slideCount; index += 1) {
    const slideStyle = emblaApi ? getSlideStyles(emblaApi, index, loop, slideCount, totalRadius) : {}
...
export const IosPickerItem = (props) => {
  ...
  return (
    <div className='embla__ios-picker'>
      <div
        className='embla__ios-picker__scene'
        ref={rootNodeRef}
      >
        <div
          ...
          >
            {slideStyles.map((slideStyle, index) => (
              <div
                ...
                key={index}
                style={wheelReady ? slideStyle : { position: 'static', transform: 'none' }}
              >
                {index}
              </div>
            ))}
